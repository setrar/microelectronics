/*
   Tft320x240.c:  Interfacing to a 320x240 TFT display
 
	Ted Rossin:	 2-15-2021
				 6-17-2022 
...
void gfSelectSDCard(void)
{
}

void gfSelectDisplay(void)
{
}
...
void gfClearDisplay(void)
{
    ...
    for(i=0;i<DISP_WIDTH*DISP_HEIGHT;i+=8){
            // Unwind loop for performance 
        gpio_put_masked(LCD_DATA_MASK,ClearColor>>8);
        gpio_put(LCD_WRN,0);
        gpio_put(LCD_WRN,1);
        gpio_put_masked(LCD_DATA_MASK,ClearColor);
        gpio_put(LCD_WRN,0);
        gpio_put(LCD_WRN,1); 
...
{	
  ...
  SetRegion(x,y,x+l-1,y);
  for(i=0;i<l;i++) LcdWriteData16(Color);
}
...
{	
  ...
  Limit = (uint8_t)l;
  SetRegion(x,y,x,l+y-1);
  for(i=0;i<Limit;i++) LcdWriteData16(Color);
}
...
{
    uint16_t i,Limit,Half;
    
    SetRegion(x,y,x+w-1,y+h-1);
        //Have to cut in half as 320*240 is larger than max of 16 bit variable
    Half = h/2;
    Limit = w*Half;
    for(i=0;i<Limit;i++) LcdWriteData16(Color);
    Limit = w*(h-Half);
...
void gfDrawRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,uint16_t Data[])
{
    ...
    for(IndexX=0;IndexX<w;IndexX++){
        for(IndexY=0;IndexY<h;IndexY++){
            LcdWriteData16(Data[IndexY+Index]);
        }
        Index+=h;
    }     
}
...
void gfDrawRect8(uint16_t x, uint16_t y, uint16_t w, uint16_t h,uint8_t Data[])
{
    ...
    for(IndexX=0;IndexX<w;IndexX++){
        Offset = 0;
        for(IndexY=0;IndexY<h;IndexY++){
            gpio_put_masked(LCD_DATA_MASK,Data[Offset+Index]);
            gpio_put(LCD_WRN,0);
            gpio_put(LCD_WRN,1);
            gpio_put_masked(LCD_DATA_MASK,Data[Offset+Index+1]);
            gpio_put(LCD_WRN,0);
            gpio_put(LCD_WRN,1);            
            Offset += 2;
        }
...
{    
    ...
    int16_t err, e2;
    uint16_t StartX,StartY,Length,Index;
    uint8_t Horizontal,Flushed;
    ...
    StartX = x0;  StartY = y0;  Length = 0;
    Horizontal = (dx>dy) ? 1 : 0;

    while(1){
            // Ugh: Painful number of writes to set a single dot
            // This is optimized to look ahead and see if adjacent pixels are written
            // on same X or Y axis and setting multiple dots after a single Set Region.
        Flushed = 0;
        if(x0==StartX || y0==StartY){
            Length++;
        }
        else{  // Line moved diagonally (dump the buffer if present)
            if(Horizontal){
                if(sx>0)
                    SetRegion(StartX,StartY,StartX+Length-1,StartY);
                else
                    SetRegion(StartX-Length+1,StartY,StartX,StartY);
            }
            else{
                if(sy>0)
                    SetRegion(StartX,StartY,StartX,StartY+Length-1);
                else
                    SetRegion(StartX,StartY-Length+1,StartX,StartY);
            }
            for(Index=0;Index<Length;Index++) LcdWriteData16(Color);
            StartX = x0;  StartY = y0;  Length = 1;  Flushed = 1;
        }
            //SetRegion(x0,y0,x0,y0);
            //LcdWriteData16(Color); 
...
{
    uint16_t FontIndex,LimitX,LimitY,Value16,SenseMask;
    ...
    if(LimitX>=DISP_WIDTH) return; 
    LimitY = Y+((ZoomFont2x) ? 2*FontHeight : FontHeight)-1;
    if(LimitY>=DISP_HEIGHT) return; 

    if(ZoomFont2x){
        SetRegion(X,Y,X+2*FontWidth-1,Y+2*FontHeight-1);
    }
    else{
        SetRegion(X,Y,X+FontWidth-1,Y+FontHeight-1);
    }
    ...
    for(Index=0;Index<FontWidth-1;Index++){
        ...
        for(ZoomLoop=0;ZoomLoop<Limit;ZoomLoop++){
            ...
            for(Bit=0;Bit<FontHeight;Bit++){
                LcdWriteData16((Value16 & SenseMask) ? FgTextColor : BgTextColor);
    ...
    Limit = FontHeight;
    if(ZoomFont2x) Limit = FontHeight*4;
    for(Bit=0;Bit<Limit;Bit++) LcdWriteData16(BgTextColor); 
}
