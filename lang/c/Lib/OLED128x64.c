...
uint8_t RawBuffer[DISP_WIDTH*DISP_HEIGHT/8+1];
uint8_t *FrameBuffer = &RawBuffer[1]; 
...
void I2cInit(void)
{
    i2c_init(i2c0,1000*1000); // 2 MHz baud rate
...
uint8_t gfUpdateDisplay(uint8_t ClearBuffer)
{
    ...
    RawBuffer[0] = 0x40;  // Set data bit of Control {C0,D/C,6'd0}

    if(i2c_write_blocking(i2c0,SLAVE_ADDR,RawBuffer,DISP_WIDTH*DISP_HEIGHT/8+1,SEND_STOP)!=(DISP_WIDTH*DISP_HEIGHT/8)+1) return(0xff);
        // I used to do this while waiting for data to xmit.
    if(ClearBuffer){
        for(Index=0;Index<DISP_WIDTH*DISP_HEIGHT/8;Index++) *ptr++ = 0;
    }    
    return(0);
}
...
uint8_t gfUpdateOneLineDisplay(uint8_t ClearBuffer,uint8_t X,uint8_t Width,uint8_t Page)
{
    ...
    SendCmd(0xb0 | Page);          // Set GDRAM Page start just low 3 bits)
    SendCmd(0x00 | (X & 0x0f));    // Set Column low nibble to 0 (low 4 bits)
    SendCmd(0x10 | ((X>>4)&0x0f));  // Set Column high nibble to 0 (low 4 bits)
    RawBuffer[0] = 0x40;  // Set data bit of Control {C0,D/C,6'd0}

    if(i2c_write_blocking(i2c0,SLAVE_ADDR,RawBuffer,Width+1,SEND_STOP)!=(Width+1)) return(0xff);
    if(ClearBuffer){
        for(Index=0;Index<Width;Index++) *ptr++ = 0;
    }   
...
void gfLine(int x0,int y0,int x1,int y1)
{
    ...
    while(1){
        if(gfColor==0) gfClearPixel(x0,y0); else gfSetPixel(x0,y0);
